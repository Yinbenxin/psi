...
TEST_P(Rr22PsiTest, CorrectTest) {
  ...
  std::vector<uint32_t> indices_psi_sender;
  ...
  PostProcessFunc receiver_post_f =
      [&](size_t, const std::vector<HashBucketCache::BucketItem>&bucket_items,
          ...
          const std::vector<uint32_t>&peer_dup_cnt) { 
            ...
            for (size_t i = 0; i < indices.size(); ++i) {
              indices_psi_sender.push_back(bucket_items[indices[i]].index);
    ...
    [&](size_t, const std::vector<HashBucketCache::BucketItem>& bucket_items,
        const std::vector<uint32_t>& indices,
        const std::vector<uint32_t>& peer_dup_cnt) {
      SPDLOG_INFO("sender_post_f: {}, {}", indices.size(),
                  peer_dup_cnt.size());
      std::unique_lock lock(mtx);
      for (size_t i = 0; i < indices.size(); ++i) {
        indices_psi_receiver.push_back(bucket_items[indices[i]].index);
        for (size_t j = 0; j < peer_dup_cnt[i]; ++j) {
          indices_psi_receiver.push_back(bucket_items[indices[i]].index);
        }
      }
  ...
  std::sort(indices_psi_sender.begin(), indices_psi_sender.end());
  ...
  SPDLOG_INFO("{}?={}", indices.size(), indices_psi_sender.size());
}
